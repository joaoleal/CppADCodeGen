#ifndef CPPAD_CODE_GEN_PRINT_OP_INCLUDED
#define CPPAD_CODE_GEN_PRINT_OP_INCLUDED
 ...
 */
template <class Base>
inline void forward_code_gen_pri_0(
std::ostream& s_out,
...
const Base* parameter) {
    const char* before;
    const char* after;
    CPPAD_ASSERT_UNKNOWN(NumArg(PriOp) == 0);
    CPPAD_ASSERT_UNKNOWN(NumRes(PriOp) == 0);
    ...
    if (arg[0] & 1) {
        ...
        pos = n.generateVarName(0, arg[1]);
    } else {
        CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
        pos = n.PrintBase(parameter[ arg[1] ]);
    }
    ...
    if (arg[0] & 2) {
        ...
        var = n.generateVarName(0, arg[3]);
    } else {
        CPPAD_ASSERT_UNKNOWN(size_t(arg[3]) < num_par);
        var = n.PrintBase(parameter[ arg[3] ]);
    }

    // after
    CPPAD_ASSERT_UNKNOWN(size_t(arg[4]) < num_text);
    after = text + arg[4];
    s_out << "if(" << pos << " <= 0) fprintf (" << n.PrintStreamName() << ", \"" << before << "%e" << after << "\", " << var << ")" << n.endl();
    
    /**
     * TODO
     * 
     * THIS HAS TO BE IMPROVED FOR ANY KIND OF DATA
     *      (it assumes double variables)
     */
