...
public:
    ...
    std::unique_ptr<LlvmModelLibrary<Base>> create() {
        ...
        std::unique_ptr<LlvmModelLibrary<Base>> lib(new LlvmModelLibrary5_0<Base>(std::move(_module), _context));
    ...
    std::unique_ptr<LlvmModelLibrary<Base>> create(ClangCompiler<Base>& clang) {
        ...
        try {
             ...
             * generate bit code
             */
            const std::set<std::string>& bcFiles = this->createBitCode(clang, "5.0");
            ...
            // voila
            lib.reset(new LlvmModelLibrary5_0<Base>(std::move(linkerModule), _context));
...
protected:
    ...
    virtual void createLlvmModule(const std::string& filename,
        ...
        std::string iClangHeaders = this->findInternalClangCHeaders("5.0", hso.ResourceDir);
