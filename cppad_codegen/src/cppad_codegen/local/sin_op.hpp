#ifndef CPPAD_CODEGEN_SIN_OP_INCLUDED
#define CPPAD_CODEGEN_SIN_OP_INCLUDED
 ...
 */
template <class Base>
inline void forward_code_gen_sin_op(
std::ostream& s_out,
...
size_t i_x) {
    ...
    if (d == 0) {
        ...
        s_out << sz_d << " = sin(" << sx << ")" << n.endl();
        s_out << sy_d << " = cos(" << sx << ")" << n.endl();
    } else {
 ...
 */
template <class Base>
inline void forward_code_gen_sin_op_0(
std::ostream& s_out,
CodeGenNameProvider<Base>& n,
size_t i_z,
size_t i_x) {
...
template <class Base>
inline void reverse_code_gen_sin_op(
std::ostream& s_out,
CodeGenNameProvider<Base>& n,
size_t d,
size_t i_z,
