#ifndef CPPAD_CODEGEN_REVERSE_CODE_GEN_SWEEP_INCLUDED
#define CPPAD_CODEGEN_REVERSE_CODE_GEN_SWEEP_INCLUDED
    ...
    while (op != BeginOp) { // next op
        ...
        switch (op) {
            ...
            case AddpvOp:
                ...
                reverse_code_gen_addpv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------
            ...
            case DivvvOp:
                reverse_code_gen_divvv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------

            case DivpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_code_gen_divpv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------

            case DivvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_code_gen_divvp_op(s_out, names, d, i_var, arg, parameter);
                break;
                // --------------------------------------------------

            case ExpOp:
                reverse_code_gen_exp_op(s_out, names, d, i_var, arg[0]);
                break;
                // --------------------------------------------------
                //			case LdpOp:
                //			reverse_code_gen_load_op(
            ...
            case LogOp:
                reverse_code_gen_log_op(s_out, names, d, i_var, arg[0]);
                break;
                // --------------------------------------------------

            case MulvvOp:
                reverse_code_gen_mulvv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------

            case MulpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_code_gen_mulpv_op(s_out, names, d, i_var, arg, parameter);
                break;
                // --------------------------------------------------
            ...
            case PowvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_code_gen_powvp_op(s_out, names, d, i_var, arg, parameter);
                break;
                // -------------------------------------------------

            case PowpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_code_gen_powpv_op(s_out, names, d, i_var, arg);
                break;
                // -------------------------------------------------

            case PowvvOp:
                reverse_code_gen_powvv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------
            ...
            case SubvvOp:
                reverse_code_gen_subvv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------

            case SubpvOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[0]) < num_par);
                reverse_code_gen_subpv_op(s_out, names, d, i_var, arg);
                break;
                // --------------------------------------------------

            case SubvpOp:
                CPPAD_ASSERT_UNKNOWN(size_t(arg[1]) < num_par);
                reverse_code_gen_subvp_op(s_out, names, d, i_var, arg);
                break;
                // -------------------------------------------------
