...
public:
    ...
    std::vector<ADCGD> model(const std::vector<ADCGD>& x,
                             const std::vector<ADCGD>& par) override = 0;
...
protected:
    ...
    void testCGAtomicGenericModelReverseOne() {
        ...
        for (size_t j = 0; j < _m; ++j) {
            py1[j] = 1;
            px1[j] = _innerModel->ReverseOne(_stdX, _stdPar, yOrig, py1);
            py1[j] = 0;
        }
    ...
    void testCGAtomicGenericModelReverseTwo() {
        ...
        for (size_t i = 0; i < _m; ++i) {
            py2[i * 2 + 1] = 1;
            px2[i] = _innerModel->ReverseTwo(_tx2, _stdPar, ty2, py2);
            py2[i * 2 + 1] = 0;
        }
    ...
    void testCGAtomicGenericModelJacobian() {
        ...
        _innerModel->SparseJacobian(_stdX, _stdPar, jacOrig, jacRow, jacCol);
    ...
    void testCGAtomicGenericModelHessian() {
        ...
        _innerModel->SparseHessian(_stdX, _stdPar, w, hessOrig, hessRows, hessCols);
