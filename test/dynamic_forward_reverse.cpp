...
#define MODEL1
    ...
    public:
        ...
        inline void compareValues(const VectorBase& depCGen,
                                  const CppAD::vector<CppAD::CG<double> >& dep,
     ...
     * static data
     */
    const std::string CppADCGDynamicForRevTest::MODEL_NAME = "dynamicForRev";
#ifdef MODEL1
...
#endif
    DynamicLibModel<double>* CppADCGDynamicForRevTest::MODEL = NULL;
    ...
    bool atomic_use_lib_for_jac_sparse(size_t id,
        ...
        const std::vector<std::set<size_t> > jacSparsity = CppADCGDynamicForRevTest::MODEL->JacobianSparsitySet();
    ...
    bool atomic_use_lib_rev_hes_sparse(size_t id,
        ...
        if (allSelected) {
            sparsitySF2R = CppADCGDynamicForRevTest::MODEL->HessianSparsitySet();
        } else {
            for (size_t i = 0; i < s.size(); i++) {
                if (s[i]) {
                    const std::vector<std::set<size_t> > sparsity = CppADCGDynamicForRevTest::MODEL->HessianSparsitySet(i);
...
TEST_F(CppADCGDynamicForRevTest, DynamicForRev) {
    using namespace std;
    using CppAD::vector;
    ...
    vector<double> yInner = CppADCGDynamicForRevTest::MODEL->ForwardZero(x);
    ...
    /*
        for (size_t j = 0; j < n; j++) {
            ...
            vector<double> y_pInner = CppADCGDynamicForRevTest::MODEL->SparseForwardOne(tx);
